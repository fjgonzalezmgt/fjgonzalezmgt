...
```mermaid
flowchart LR
    A[Problema operativo] --> B[Recurso técnico]
    B --> C[Criterio de aplicación]
    C --> D[Decisión mejor sustentada]
```
...
```mermaid
flowchart LR
    A[Biblioteca técnica] --> B[Criterios]
    B --> C[Notebooks]
    B --> D[Dashboards]
    B --> E[Apps]
    B --> F[Automatizaciones]
    C --> G[Uso operativo]
    D --> G
    E --> G
    F --> G
    G --> H[Mejores decisiones]
```
